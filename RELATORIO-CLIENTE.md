...
### Multi-CV — citar o CV concreto, nunca "(CV)" genérico (gap simulação 2026-06-18)
Onde o parecer hoje escreve "(CV)", passa a **citar o `document` de referência**:
**filename + version + data**. (*"o CV diz 3 anos"* → *"o CV `joao-silva-v2.pdf`
(versão 2, 12/06) diz 3 anos"*.)
- O `vs_cv` mede **sempre** contra o CV **`is_current` com `source='uploaded'`** (ou
  `'attested'`) — o CHECK `doc_is_current_only_uploaded` garante-o no schema
  (`MODELO-DADOS §16.C`). **Nunca** se mede o candidato contra um CV **gerado pela Vera**.
- Quando há vários CVs, o parecer torna explícito **contra qual** mediu — desambigua
  divergências que de outro modo pareceriam contradições do candidato.

### Selo obrigatório no CV GERADO enviado ao cliente (gap simulação 2026-06-18)
Qualquer `document source='generated'` (CV reformatado pela ferramenta) que seja enviado
ao cliente leva, no rodapé, o selo:
> *"Reformatado pela ferramenta a partir do CV original (disponível)."*
- Guarda `based_on_document_id` a apontar para o CV-fonte (`MODELO-DADOS §16.C`) — o
  cliente pode sempre pedir/ver o original.
- Coerente com a regra anti-difamação: uma divergência candidato↔CV-**gerado** é
  `divergence_origin='vera_generated'` e **não** conta como inconsistência do candidato.
...
### Parecer STALE — correção a montante invalida um `report` já enviado
Uma correção a montante (re-atribuição de falante §2, novo CV de referência, edição de
factos) pode invalidar um `report` que já estava `ready`. Quando isso acontece, marca-se
`report.invalidated_at` + `stale_reason` (`MODELO-DADOS §16.B`) e:
- **Alerta à Filipa** — *"o parecer do João mudou depois de corrigires quem falou aos
  34:12; revê antes de usar"*.
- **Se ainda NÃO foi enviado** (`client_sent_at IS NULL`): basta re-gerar antes de enviar.
- **Se JÁ foi enviado** (`client_sent_at` preenchido): a Vera **sugere enviar uma
  correção ao cliente** — e esse envio passa pela **mesma porta anti-envio-mal-endereçado**
  acima (validação de domínio), porque continua a levar PII ao cliente.
