...
**Memória genérica reutilizável (CORE-memória):** `agents/memory_agent.py`, `agents/entity_memory.py` (per-candidato em vez de per-cliente), `agents/semantic_memory.py` (RAG — ⚠️ ver nota abaixo), `agents/procedural_memory.py`. Guardrails genéricos: `agents/self_defense.py` (regex anti-injeção), `agents/prompt_shield.py` (classificador LLM de manipulação). Loop de aprendizagem: `agents/outcome_reviewer.py`, `agents/hypothesis_tracker.py`. Escalonamento: `agents/crisis_handler.py` (**trocar Telegram por canal RH**).

> 🔒 **CORREÇÃO de segurança (R2) — NÃO clonar o RAG tal-qual:** o `semantic_memory.py` da origem usa **ChromaDB com uma coleção GLOBAL sem `agency_id`** (+ fallback SQL também sem tenant) → na v2 multi-agência **vaza CV/cliente entre agências** pela porta do RAG (`SEGURANCA §1`, `MODELO-DADOS §15.9/§15.11`). Na Vera, **re-apontar o RAG a pgvector no Postgres** (1 store, sujeito ao funil `agency_id` + GUC do agente) — ou, se Chroma ficar, **coleção-por-agência** + `where={"agency_id":…}` obrigatório, **nunca** coleção partilhada. A purga RGPD tem de atingir este store (não fica PII órfã no índice).
