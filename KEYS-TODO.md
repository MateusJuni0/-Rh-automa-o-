...
| **Supabase Auth** | self-hosted local — `@supabase/ssr` (config-not-code) | `apps/web/lib/session.ts` (`getSession`) + `lib/api.ts` + `lib/auth.ts` + `app/login` + `middleware.ts` | login real email/senha | ✅ **stack local VALIDADO** (Ω-2 B): `--profile supabase` sobe (auth/rest/storage/kong:8000), Filipa+Inês seeded no GoTrue, login email/senha ponta-a-ponta OK (password certa→token, errada→400). Passo-a-passo de subida no topo do `docker-compose.dev.yml`. Para PRODUÇÃO falta o Mateus dar o `SUPABASE_*` real (URL/anon/service/jwt-secret próprios). |
...
- ✅ **`argsSchema` por tool** no assistente — FEITO (Ω-2 A3): cada `ToolDef` tem `argsSchema` (Zod); validado ao planear (`run.ts` descarta tool-call inválida) e antes de executar (`gate.ts` → `invalid_args`) e re-validado na confirmação. `enviar_email.to` validado contra `EMAIL_DOMAIN_ALLOWLIST` server-side (anti-exfiltração por prompt-injection). ⚠️ Quando ligar o adapter REAL de email: validar TAMBÉM cc/bcc contra a allowlist (o `.loose()` deixa-os passar; o mock não envia, mas o real tem de os filtrar).
- ✅ **Invariantes no arranque (fail-fast):** FEITO (Ω-2 A4): `assertAuthConfig()` (em `lib/auth-config.ts`, chamado por `instrumentation.ts` no boot) aborta se URL/ANON_KEY só um dos dois, ou se produção sem auth e sem `ALLOW_DEV_SESSION=1`. O fallback DEV de `getSession` exige `ALLOW_DEV_SESSION` (não infere de `NODE_ENV`).
- ✅ **Cookies do shim:** FEITO (Ω-2 A4): `secure: isHttps(req)` (de `x-forwarded-proto`/URL, não `NODE_ENV`) + `sameSite: "strict"`.
