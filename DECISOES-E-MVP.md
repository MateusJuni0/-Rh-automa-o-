...
- **NOVO `LEGAL-E-RGPD.md`** (estava AUSENTE): 🔴 DPA Art.28 (CMTec=subcontratante IRIS, pré-req do piloto) · consentimento do candidato (texto+prova `process.consent_status`+caminho "sem captura"+Art.22 assistivo) · cláusula reutilização cross-cliente · cifra+purga na migração · tabela de retenção (dias, proposta) · IP/licença/code-signing/responsabilidade · Apify=tokens do comprador · anti-spoof gate. Marcado o que é 🟦 Mateus/jurídico.
